...
get_archpkg (){
  ...
  wget https://aur.archlinux.org/packages/en/electrum-oracoin-git/electrum-oracoin-git.tar.gz
  tar -xpzvf electrum-oracoin-git.tar.gz
  sed -e 's/_gitbranch\=.*/_gitbranch='${archbranch}'/g' electrum-oracoin-git/PKGBUILD > electrum-oracoin-git/PKGBUILD.new
  mv electrum-oracoin-git/PKGBUILD.new electrum-oracoin-git/PKGBUILD
  rm electrum-oracoin-git.tar.gz
  popd
}
prepare_repo(){
...
buildRelease(){
  ...
  # echo "Making locales" 
  # $DOCKERBIN run --rm -it --privileged -e MKPKG_VER=${VERSION} -v $(pwd)/helpers:/root  -v $(pwd)/repo:/root/repo  -v $(pwd)/source:/opt/wine-electrum/drive_c/electrum-oracoin/ -v $(pwd):/root/electrum-oracoin-release oracoin/electrum-oracoin-release:${VERSION} /bin/bash
...
build_Windows(){
   echo "Making Windows EXEs for $VERSION" \
    ...
    && ls -la $(pwd)/helpers/release-packages/Windows/Electrum-Oracoin-${VERSION}-Windows-setup.exe \
...
buildImage(){
  echo "Building image"
  case "${1}" in 
  winbuild) $DOCKERBIN build -t oracoin/electrum-oracoin-winbuild:${VERSION} .
         ;;
   release) $DOCKERBIN build -f Dockerfile-release -t  oracoin/electrum-oracoin-release:${VERSION} .
         ;;
  esac
}
...
prepareFile(){
  echo "Preparing file for Electrum-Oracoin version $VERSION"
  if [ -e "$TARGETPATH" ]; then
    echo "Version tar already downloaded."
  else
   wget https://github.com/oracoin/electrum-oracoin/archive/v${VERSION}.zip -O $TARGETPATH
  fi
