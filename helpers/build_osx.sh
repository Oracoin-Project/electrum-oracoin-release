...
if [ "$(uname)" = "Darwin" ];
  ...
  mv dist/Electrum-Oracoin.app ../src/ 
  cd ..
  #make  -  makes the unneeded dmg
  test -d helpers/release-packages/OSX || mkdir -pv helpers/release-packages/OSX
  #mv Electrum-Oracoin-${VER}.dmg helpers/release-packages/OSX
  mv src/Electrum-Oracoin.app helpers/release-packages/OSX
  cp helpers/make_OSX-installer.sh helpers/release-packages/OSX
  pushd helpers/release-packages/OSX
  mkdir Electrum-Oracoin.app/Contents/Resources/lib/python2.7/site-packages
  pushd  Electrum-Oracoin.app/Contents/Resources/lib/python2.7/site-packages
