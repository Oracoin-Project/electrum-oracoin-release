...
if [[ $# -gt 0 ]]; then
  ...
  export FILENAME=Electrum-Oracoin-$VERSION.zip
  echo "export FILENAME=Electrum-Oracoin-$VERSION.zip" >> build-config.sh
  export TARGETPATH=$(pwd)/source/$FILENAME
  echo "export TARGETPATH=$(pwd)/source/$FILENAME" >> build-config.sh
  export TARGETFOLDER=$(pwd)/source/Electrum-Oracoin-$VERSION
  echo "export TARGETFOLDER=$(pwd)/source/Electrum-Oracoin-$VERSION" >> build-config.sh
  echo "Building Electrum-Oracoin $VERSION from $FILENAME"
else
